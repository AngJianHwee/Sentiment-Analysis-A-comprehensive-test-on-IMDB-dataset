...
# -----------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------
# train test split add label save to column "train_test_flag"
import random
...
for approach in tqdm(VECTORIZATION_APPROACHES, desc='Approach', position=0):
    ...
    for cur_model in tqdm(CLASSIFICATION_MODELS, desc='Model', position=1):
        ...
        train_results[approach][cur_model[1]] = run_model(cur_model, approach, X_train, y_train, X_test, y_test,logger)
